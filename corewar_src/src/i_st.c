...
int exec_st(unsigned char *mem, int pc, t_process *process)
{
	...
	else if(mem[pc + 1] == 0x70)
	{
		...
		if (*arg1 < 0x10)
			ft_memcpy(&mem[MMS(pc + MODFIX(hatole(arg2, 2), IDX_MOD))], process->reg[GET_REGNB(arg1)], REG_SIZE);
		else
			return (1);
		process->pc += 5;
	}
	...
	ft_putstr("\nmem[");
	ft_putnbr(MMS(pc + MODFIX(hatole(arg2, 2), IDX_MOD)));
	ft_putstr("]: ");
	ft_print_memory(&mem[MMS(pc + MODFIX(hatole(arg2, 2), IDX_MOD))], REG_SIZE);
